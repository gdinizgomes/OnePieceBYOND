...
mob
    ...
    var/real_x = 0;
    var/real_y = 0; // NOVA VARIAVEL: Altura (Eixo Y)
    ...
    proc/LoadCharacter(slot)
        ...
        F["max_hp"] >> src.max_hp
        F["pos_x"] >> src.real_x;
        if(F["pos_y"]) F["pos_y"] >> src.real_y; // Carrega o Y se existir
        F["pos_z"] >> src.real_z
        F["skin"] >> src.skin_color;
        F["cloth"] >> src.cloth_color
        return 1
    ...
    Topic(href, href_list[])
        ...
        if(action == "create_char")
            ...
            src.level = 1;
            src.gold = 0; src.real_x = 0; src.real_y = 0; src.real_z = 0
            current_slot = slot;
        ...
        if(action == "update_pos" && in_game)
            ...
            real_y = text2num(href_list["y"]); // Recebe o Y
