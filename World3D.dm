...
mob
	...
	proc/LoadCharacter(slot)
		...
		if(F["str"]) F["str"] >> src.strength
		else src.strength = 5
		if(F["vit"]) F["vit"] >> src.vitality
		else src.vitality = 5
		if(F["agi"]) F["agi"] >> src.agility
		else src.agility = 5
		if(F["wis"]) F["wis"] >> src.wisdom
		else src.wisdom = 5

		if(F["p_punch"]) F["p_punch"] >> prof_punch_lvl
		else prof_punch_lvl = 1
		if(F["exp_punch"]) F["exp_punch"] >> prof_punch_exp
		if(F["p_kick"]) F["p_kick"] >> prof_kick_lvl
		else prof_kick_lvl = 1
		if(F["exp_kick"]) F["exp_kick"] >> prof_kick_exp
		if(F["p_sword"]) F["p_sword"] >> prof_sword_lvl
		else prof_sword_lvl = 1
		if(F["exp_sword"]) F["exp_sword"] >> prof_sword_exp
		if(F["p_gun"]) F["p_gun"] >> prof_gun_lvl
		else prof_gun_lvl = 1
		...
		if(action == "attack" && in_game)
			...
			if(ConsumeEnergy(base_cost))
				...
				if(attack_type == "fist")
					...
					prof_bonus = prof_lvl * 2
				if(attack_type == "kick")
					prof_kick_lvl = prof_kick_lvl
					prof_bonus = prof_lvl * 2
				if(attack_type == "sword")
