			...
			try
				...
				for(var/mob/M in active_clients)
					...
					for(var/id in all_entity_data)
						...
						if(sqrt((edata["x"] - M.real_x)**2 + (edata["z"] - M.real_z)**2) <= 40.0) my_visible_entities[id] = edata
					
					var/list/my_visible_ground = list()
					if(send_ground)
						for(var/list/gdata in ground_data)
							if(sqrt((gdata["x"] - M.real_x)**2 + (gdata["z"] - M.real_z)**2) <= 40.0) my_visible_ground += list(gdata)
					...
					if(global_events.len > 0)
						...
						for(var/list/evt in global_events)
							if(sqrt((evt["x"] - M.real_x)**2 + (evt["z"] - M.real_z)**2) <= 40.0) my_global_json_list["evts"] += list(evt)
