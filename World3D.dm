...
var/global/datum/database_manager/db_manager
...
datum/database_manager
    ...
    proc/Connect()
        ...
        db = new
        try
            if(db && db.Connect("DBI:mysql:database=onepiece;host=127.0.0.1;port=3306", "onepiece", "onepiece"))
                connected = 1
        catch
            connected = 0

    proc/SaveCharacter(mob/M)
        if(!connected || !db || !M) return 0
        var/query = {"
            REPLACE INTO characters
            (ckey, slot, name, level, gold, hp, max_hp, pos_x, pos_z, skin, cloth, fruit)
            VALUES ('[M.ckey]', [M.current_slot], '[M.name]', [M.level], [M.gold], [M.current_hp], [M.max_hp],
            [M.real_x], [M.real_z], '[M.skin_color]', '[M.cloth_color]', '[M.fruit_type]')"}
        db.Query(query)
        return 1

    proc/LoadSlotSummary(ckey, slot)
        if(!connected || !db) return null
        var/query = "SELECT name, level, gold FROM characters WHERE ckey='[ckey]' AND slot=[slot] LIMIT 1"
        var/database/recordset/R = db.Query(query)
        if(!R || !R.NextRow()) return null
        return list("name" = R.GetString(1), "lvl" = text2num(R.GetString(2)), "gold" = text2num(R.GetString(3)))

    proc/LoadCharacter(mob/M, slot)
        if(!connected || !db || !M) return 0
        var/query = "SELECT name, level, gold, hp, max_hp, pos_x, pos_z, skin, cloth, fruit FROM characters WHERE ckey='[M.ckey]' AND slot=[slot] LIMIT 1"
        var/database/recordset/R = db.Query(query)
        if(!R || !R.NextRow()) return 0
        M.name = R.GetString(1)
        M.level = text2num(R.GetString(2))
        M.gold = text2num(R.GetString(3))
        M.current_hp = text2num(R.GetString(4))
        M.max_hp = text2num(R.GetString(5))
        M.real_x = text2num(R.GetString(6))
        M.real_z = text2num(R.GetString(7))
        M.skin_color = R.GetString(8)
        M.cloth_color = R.GetString(9)
        M.fruit_type = R.GetString(10)
        return 1
...
mob
    ...
    proc/SaveCharacter()
        ...
        var/saved_to_db = 0
        if(db_manager && db_manager.connected)
            saved_to_db = db_manager.SaveCharacter(src)
        if(!saved_to_db)
